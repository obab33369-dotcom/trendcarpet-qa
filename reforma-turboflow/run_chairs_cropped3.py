...
FTP_UPLOAD_DIR = os.path.join(ONEDRIVE_DIR, "turboflow", "chairs_cropped3")
...
def crop_and_resize(img_path, target_size=2000, slot=1, is_zoom=False):
    with Image.open(img_path) as img:
        ...
        if bbox is not None:
            ...
            if is_zoom:
                ...
                # Main image (slot 1): custom dining chair crop/scaling (symmetrical floor aligned)
...
def main():
    print("==================================================")
    print("   CHAIRS CROPPED3 (35% ENLARGEMENT) PIPELINE")
    print("==================================================")
    ...
    for idx, (cat, r_folder, prod_path) in enumerate(render_folders):
        ...
        if 1 in selected_white_bg:
            ...
            main_slot = min(selected_white_bg.keys())
            main_img = selected_white_bg[main_slot]
        ...
        try:
            # 1000px / 400px: 35% bigger
            main_normal_resized = crop_and_resize(os.path.join(prod_path, main_img), target_size=1000, slot=main_slot, is_zoom=False)
            ...
            # # 2000px: 30% bigger
            # main_zoom_resized = crop_and_resize(os.path.join(prod_path, main_img), target_size=2000, slot=main_slot, is_zoom=True)
            ...
            #     resized_img = crop_and_resize(src_path, target_size=2000, slot=slot_num, is_zoom=True)
    ...
    print("\n==================================================")
    print("          CHAIRS CROPPED3 PIPELINE COMPLETED!     ")
    print("==================================================")
