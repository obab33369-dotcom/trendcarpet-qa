...
print("Analyzing slots for stol-alta-natur:")
for slot in range(1, 11):
    filename = f"0{97+slot}_stol-alta-natur-{slot}-26U-wonder.webp"
    path = os.path.join(topaz_dir, filename)
    if os.path.exists(path):
        mean, std, color = analyze_img(path)
        print(f"  Slot {slot}: Mean={mean:.1f}, Std={std:.2f}, Color={color.round(1)}")
    else:
        print(f"  Slot {slot}: File {filename} not found.")
