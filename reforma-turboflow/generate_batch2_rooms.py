...
def main():
    ...
    # Set of unfeatured product IDs (starts with all 173 items)
    unfeatured_ids = set(db.keys())
    
    # List to store matched packages
    packages = []
    
    # Each active item should be featured as a seed exactly 2 times
    seeds = sorted(list(db.keys())) * 2
    ...
    for seed_id in seeds:
        ...
        if matched_pkg:
            ...
            # Fallback minimal package to ensure seeds never crash
    ...
    for pkg_idx, pkg in enumerate(packages):
        ...
        prompt_1 = (
            f"{shot1_num:03d} - {PREFIX}Architectural digest-style view of {subject_desc} set in a {location_desc} with {shot1_light}{focus_phrase}.{bg_part_1} "
        ...
        prompt_2 = (
            f"{shot2_num:03d} - {PREFIX}Architectural digest-style close-up shot showcasing {subject_desc_2}, set in a {location_desc} with {shot2_light}{focus_phrase}.{bg_part_2} "
            f"Dynamic three-quarter angle with controlled depth of field. Varied gloss levels on walls and trim to enhance depth, featuring tactile micro-imperfections. "
            f"Strictly preserve individual furniture material finishes (matte, lacquer, metal) without global gloss changes. Enhance atmosphere with very subtle volumetric-dust. "
            f"Prioritize high-end material tactility and realistic light-reactive surfaces over digital polish, showcasing premium wood and stone surfaces. "
            f"Utilize chiaroscuro lighting techniques to deepen shadow definition, ensuring clear silhouettes for furniture against the walls. "
        ...
        # Shot 3 and Shot 4 have been removed as per user request to generate only 2 shots (normal + close-up) per package.
