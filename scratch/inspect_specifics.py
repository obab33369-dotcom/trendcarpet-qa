...
def inspect_image(img_path, key):
    ...
    prompt = (
        ...
        "1. List each card you see from top to bottom (or left to right, depending on layout). "
        "2. What digit is written on each card? "
        "3. Read the digits as a single number and print the final number. "
        "Please be extremely precise. Do not repeat digits or read background text."
    )
