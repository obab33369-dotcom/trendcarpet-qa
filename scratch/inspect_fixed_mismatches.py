...
for md5, instances in md5_groups.items():
    ...
    if len(sku_to_files) > 1:
        ...
        if len(unique_names) > 1:
            ...
            entry = f"[MISMATCH DETECTED] Hash: {md5}\n"
            for sku, files_info in sku_to_files.items():
                entry += f"  * SKU: {sku}\n"
                for uf, name in files_info:
                    entry += f"    - File: {uf} -> Resolved to: {name}\n"
            entry += "-" * 50 + "\n"
            output_lines.append(entry)

summary_path = os.path.join(WORKSPACE_DIR, "scratch", "fixed_mismatches_summary.txt")
with open(summary_path, 'w', encoding='utf-8') as fh:
    fh.write(f"Found {true_rug_mismatches} true cross-product design mismatches.\n\n")
    fh.writelines(output_lines)

print(f"CONCISE SUMMARY: Found {true_rug_mismatches} true design mismatches. Details saved to scratch/fixed_mismatches_summary.txt")
