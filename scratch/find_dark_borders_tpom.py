            ...
            if f.lower().endswith(('.jpg', '.jpeg', '.png')):
                ...
                try:
                    with Image.open(p_file) as img:
                        ...
                        # Use RGB mean < 120 to find dark/gray borders
                        dark_top = np.mean(top_border, axis=-1) < 120
                        dark_bottom = np.mean(bottom_border, axis=-1) < 120
                        dark_left = np.mean(left_border, axis=-1) < 120
                        dark_right = np.mean(right_border, axis=-1) < 120
...
find_dark_borders(r"C:\Users\AndronikLindgren\OneDrive - CaMa Gruppen AB\Pictures\turboflow\tpom\artiklar")
