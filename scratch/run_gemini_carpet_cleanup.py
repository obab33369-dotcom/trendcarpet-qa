...
def process_folder(folder_name):
    ...
    if not ref_image:
        print(f"Skipping '{folder_name}' - no reference image found.")
        return
    ...
    for path in candidates:
        ...
        # Determine if file is currently in reserv or main folder
        in_reserv = "reserv" in os.path.dirname(path)
        
        if not is_keep:
            discarded_count += 1
            os.makedirs(dest_discard_folder, exist_ok=True)
            # Copy reference photo to discard folder
            ...
            # Move candidate file to discard folder (into a reserv subfolder if it was in reserv)
            if in_reserv:
    ...
    if discarded_count > 0:
        print(f"Processed '{folder_name}': Kept {kept_count}, Discarded {discarded_count} images.")
    else:
        print(f"Processed '{folder_name}': Kept {kept_count} images.")

def run_all_cleanup():
    if not API_KEY:
        print("Error: GEMINI_API_KEY env variable is not set!")
        return
        
    print("Starting Gemini visual cleanup across all carpet folders...")
    folders = [f for f in os.listdir(SOURCE_DIR) if os.path.isdir(os.path.join(SOURCE_DIR, f))]
    print(f"Found {len(folders)} folders to process.")
    
    # Run with ThreadPoolExecutor
    # 15 concurrent threads to optimize API requests speed
    ...
    print("Gemini carpet cleanup completed successfully.")
